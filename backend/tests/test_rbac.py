...
async def test_me_permissions_endpoint(client: AsyncClient, db_session):
    ...
    # ── Test 1: Owner gets all 41 permissions ──
    ...
    # ── Test 2: Teacher gets exactly 29 permissions ──
    res = await client.get("/api/v1/roles/me/permissions", headers=headers_teacher)
    assert res.status_code == 200
    teacher_perms = res.json()["permissions"]
    assert len(teacher_perms) == 29
    assert "workspace.read" in teacher_perms
    assert "subject.create" in teacher_perms
    assert "subject.material.create" in teacher_perms
    assert "subject.material.read" in teacher_perms
    assert "ai.use" in teacher_perms
    assert "assignment.read" in teacher_perms
    assert "assignment.create" in teacher_perms
    assert "assessment.create" in teacher_perms
    assert "assessment.read" in teacher_perms
    assert "assessment.publish" in teacher_perms
    assert "question.create" in teacher_perms
    assert "question.read" in teacher_perms
    assert "question.reorder" in teacher_perms
    assert "member.create" not in teacher_perms
    assert "settings.manage" not in teacher_perms

    # ── Test 3: Student gets exactly 8 permissions ──
    res = await client.get("/api/v1/roles/me/permissions", headers=headers_student)
    assert res.status_code == 200
    student_perms = res.json()["permissions"]
    assert len(student_perms) == 8
    assert set(student_perms) == {"workspace.read", "attendance.view", "ai.use", "subject.read", "subject.material.read", "assignment.read", "assessment.read", "question.read"}
    ...
    # Multi user in Org A (Teacher) → 29 permissions
    res_a = await client.get("/api/v1/roles/me/permissions", headers=headers_multi)
    assert res_a.status_code == 200
    assert len(res_a.json()["permissions"]) == 29

    # Multi user in Org B (Student) → 8 permissions
    ...
    assert len(res_b.json()["permissions"]) == 8
    assert set(res_b.json()["permissions"]) == {"workspace.read", "attendance.view", "ai.use", "subject.read", "subject.material.read", "assignment.read", "assessment.read", "question.read"}
